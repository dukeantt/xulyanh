...
image = cv2.imread('image/ratnhieumeo3.jpg', 1)
...
#image 1 threshold
#image 2 thresh0ld 110
#image 3 threshold
#image 4 threshold
threshold_value = 115
thresh = cv2.adaptiveThreshold(grayImage, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 199, 27)
...
img_erode = cv2.dilate(img_erode, kernel, iterations=1)
img_erode = cv2.dilate(img_erode, kernel, iterations=1)
# img_erode = cv2.dilate(img_erode, kernel, iterations=1)


# img_erode = cv2.erode(img_erode, kernel2, iterations=1)
...
# Labeling
ret2, labels = cv2.connectedComponents(img_erode)
